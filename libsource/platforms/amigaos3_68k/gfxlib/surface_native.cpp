...
*/
Display::Surface::Surface(BitMap* bmp, uint32 flags, sint16 w, sint16 h) : AbstractPixelMap()
{
  init();
  bitMap      = bmp;
  properties  = flags;
...
void Display::Surface::allocateBitMap(OSNative::BitMap* clone, sint16 w, sint16 h)
{
  LOGGING_DECLARE_FUNCNAME(allocate)
  assertFree();
  bitMap  = createBitMap(clone, w, h);
  width   = w;
  height  = h;
  queryBitMap();
...
void Display::Surface::releaseBitMap()
{
  LOGGING_DECLARE_FUNCNAME(releaseBitMap)
  ...
  if (bitMap) {
    if (lockAddr) {
      ...
      SystemLog::write(
        SystemLog::INFO,
        "%s::%s - unlocked BitMap at %p\n",
        loggingClassName, loggingFuncName, bitMap
      );
      #endif
    }
...
BitMap* Display::Surface::createBitMap(BitMap* clone, sint16 w, sint16 h)
{
  LOGGING_DECLARE_FUNCNAME(createBitMap)
  BitMap* bitMap = 0;
  if (w < 0 || h < 0) {
  ...
  SystemLog::write(
    SystemLog::INFO,
    "%s::%s - AllocBitmap(%" XPF_S16 ", %" XPF_S16 ", 8, 0x%08X, %p) returned CGX BitMap at %p\n",
    loggingClassName, loggingFuncName,
    w, h, (unsigned)bmf, clone, bitMap
  );
  #endif
...
Display::Surface* Display::Surface::createFromBitMap(OSNative::BitMap* bmp)
{
  LOGGING_DECLARE_FUNCNAME(createFromBitMap)

  assertRTG(bmp);
  Surface* newSurface = new (nothrow) Surface(bmp, SP_EXT_BITMAP, GetCyberMapAttr(bmp, CYBRMATTR_WIDTH), GetCyberMapAttr(bmp, CYBRMATTR_HEIGHT));
  if (!newSurface) {
    THROW_NSX(Mem, Exhausted(sizeof(Surface)));
  }
  return newSurface;
}
...
Display::Surface* Display::Surface::createLikeBitMap(OSNative::BitMap* bmp, sint16 w, sint16 h)
{
  LOGGING_DECLARE_FUNCNAME(createLikeBitMap)
  assertRTG(bmp);
  Surface* newSurface = new (nothrow) Surface(createBitMap(bmp, w, h), SP_OWN_BITMAP, w, h);
  if (!newSurface) {
    THROW_NSX(Mem, Exhausted(sizeof(Surface)));
  }
  return newSurface;
...
Display::Surface* Display::Surface::create(Display::Surface* clone, sint16 w, sint16 h)
{
  ...
  return createLikeBitMap(clone->bitMap, w, h);
}
