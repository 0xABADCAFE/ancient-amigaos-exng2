...
// Static data
Task*   Thread::main        = IExec->FindTask(0);
...
void Thread::sharedInit(uint32 stack)
{
  ...
  numThreads++;
  if (!internal) {
    parent = threadForTask(IExec->FindTask(0)/*FindTask(0)*/);
  }
  else {
    parent = 0;
  }
...
Thread::~Thread()
{
  ...
  if (internal) {
    ...
    external = IExec->FindTask(0);//FindTask(0);
    taskState |= TASKSTATE_KILL;
    signalInternal(SIGNAL_STOP);
    while (taskState & TASKSTATE_KILL) {
      if (IExec->Wait(SIGNAL_ACK|SIGNAL_SYSBREAK)==SIGNAL_SYSBREAK) {
        taskState &= ~TASKSTATE_KILL;
    ...
    #ifdef EXNG2_BUILD_LOGGED
    SystemLog::write(
      SystemLog::WARNING, "%s::%s - %s terminated\n",
      loggingClassName, loggingFuncName, name
    );
    #endif
  }
...
Thread::Priority Thread::setPriority(Priority p)
{
  ...
  if (oldPri !=p) {
    priority = (uint8)p;
    if (internal) {
      IExec->SetTaskPri(&(internal->pr_Task), priTab[priority]);
    }
  }
  return oldPri;
...
void Thread::start()
{
  ...
  #ifdef EXNG2_BUILD_LOGGED
  SystemLog::write(
    SystemLog::INFO, "%s::%s - %s signalling process @ %p\n",
    loggingClassName, loggingFuncName, name, internal
  );
  #endif
  ...
  #ifdef EXNG2_BUILD_LOGGED
  SystemLog::write(
    SystemLog::INFO, "%s::%s - %s spawn new process\n",
    loggingClassName, loggingFuncName, name
  );
  #endif
...
{
  IExec->Signal(&(internal->pr_Task),trigger);
}
...
void Thread::spawn()
{
  LOGGING_DECLARE_FUNCNAME(spawn)
    ...
    #ifdef EXNG2_BUILD_LOGGED
    SystemLog::write(
      SystemLog::INFO, "%s::%s - %s attempting spwan\n",
      loggingClassName, loggingFuncName, name
    );
    #endif
  ...
  external  = IExec->FindTask(0);//FindTask(0);
  if (!(internal = IDOS->CreateNewProc(spawnTags))) {
    taskState = 0;
  ...
  #ifdef EXNG2_BUILD_LOGGED
  SystemLog::write(
    SystemLog::ERROR, "%s::%s - %s created Process at %p\n",
    loggingClassName, loggingFuncName, name, internal
  );
  #endif
...
void  Thread::entryPoint()
{
  ...
  #ifdef EXNG2_BUILD_LOGGED
  Task* t = IExec->FindTask(0);//FindTask(0);
  SystemLog::write(
    SystemLog::INFO, "%s::%s - Task at %p initializing\n",
    loggingClassName, loggingFuncName, t
  );
  #endif
  ...
  try {
    ...
    Task* t = IExec->FindTask(0);//FindTask(0);
    SystemLog::write(
      SystemLog::ERROR, "%s::%s Task %p encountered unexpected %s\n",
      loggingClassName, loggingFuncName, t, e.getClass()
    );
    #endif
    ...
    Task* t = IExec->FindTask(0);//FindTask(0);
    SystemLog::write(
      SystemLog::ERROR, "%s::%s - Unknown Task %p\n",
      loggingClassName, loggingFuncName, t
    );
    #endif
    ...
    while (!(taskState & TASKSTATE_KILL)) {
     ...
     SystemLog::write(
      ...
      if (taskState & (TASKSTATE_STOP|TASKSTATE_JOIN)) {
       ...
       #ifdef EXNG2_BUILD_LOGGED
       SystemLog::write(
         SystemLog::INFO, "%s::%s - %s [i] received stop|join\n",
         loggingClassName, loggingFuncName, name
       );
       #endif
      ...
      while ((taskState & (TASKSTATE_PARKED|TASKSTATE_KILL))==TASKSTATE_PARKED) {
       ...
       SystemLog::write(
        ...
        IExec->Wait(SIGNAL_ACK|SIGNAL_STOP);

       #ifdef EXNG2_BUILD_LOGGED
       SystemLog::write(
         SystemLog::INFO, "%s::%s - %s [i] parked state interrupted by ack|stop\n",
         loggingClassName, loggingFuncName, name
       );
       #endif
