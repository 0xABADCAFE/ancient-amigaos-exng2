...
void SystemLog::init()
{
  ...
  if (!(logSemaphore = (SignalSemaphore*) IExec->AllocMem(sizeof(SignalSemaphore), MEMF_SHARED))) {
    THROW_NSX(Error, ResourceExhausted("logbuffer"));
  }
  IExec->InitSemaphore(logSemaphore);
...
void SystemLog::done()
{
  ...
  if (logSemaphore) {
    IExec->FreeMem(logSemaphore, sizeof(SignalSemaphore));
  }
...
void SystemLog::write(Level level, const char* msg, ...)
{
  ...
  IExec->ObtainSemaphore(logSemaphore);
  ...
  IExec->ReleaseSemaphore(logSemaphore);
}
