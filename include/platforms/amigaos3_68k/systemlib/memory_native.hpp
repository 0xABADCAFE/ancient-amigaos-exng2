...
class RawMemHandler {
  protected:
    ...
    static bool   validAllocator()  { return memPool!=0;                          }
    static void   lockAllocator()   { OSNative::ObtainSemaphore(poolSemaphore);   }
    static void   unlockAllocator() { OSNative::ReleaseSemaphore(poolSemaphore);  }
...
{
  return OSNative::AvailMem(MEMF_FAST|MEMF_LARGEST);
}
