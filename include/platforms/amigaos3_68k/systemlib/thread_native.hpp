...
class Thread : protected Runnable {
	...
	public:
		...
		class ThreadError;
		class ThreadSecurityViolation;
		class ThreadStateViolation;
		class ThreadStartupFailure;
	...
	public:
		...
		// mutators
		void					setRunObserver(ThreadRunObserver* observer);
		void					setStateObserver(ThreadStateObserver* observer);
	...
	private:
		...
		Thread*									parent;
		Runnable*								runJob;
		ThreadRunObserver*			runObs;
		ThreadStateObserver*		stateObs;
		Sleeper									sleeper;
		Clock										uptime;
		uint32									stackSize;
		uint8										priority;
